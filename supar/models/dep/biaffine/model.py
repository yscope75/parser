...
class BiaffineDependencyWAttentionsModel(Model):
    r"""
    The implementation of Biaffine Dependency Parser :cite:`dozat-etal-2017-biaffine`.
    With transformer attentions enhanced
    Args:
        n_words (int):
            The size of the word vocabulary.
        n_rels (int):
    ...
    def loss_with_attn(self, s_arc, s_rel, arcs, rels, attn_s_arc, attn_s_rel, mask, partial=False):
        return 0.5*self.loss(s_arc, s_rel, arcs, rels, mask, partial) + \
            0.5*self.loss(attn_s_arc, attn_s_rel, arcs, rels, mask, partial)
